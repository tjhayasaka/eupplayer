...
class PolyphonicAudioDevice
{
  ...
  int _volume;
 public:
  PolyphonicAudioDevice() { _timeStep.tv_sec = _timeStep.tv_usec = 0; _volume = 256; }
  virtual ~PolyphonicAudioDevice() {}
  virtual struct timeval const &timeStep() const { return _timeStep; }
  virtual int volume(int val = -1) { if (val >= 0) _volume = val; return _volume; }
  virtual void timeStep(struct timeval const &step) { _timeStep = step; }
